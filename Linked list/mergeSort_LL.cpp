...
void break_LL(Node* head,Node* h1,Node* h2){
    Node* sp = head;
    Node* fp = head;

    while(fp != NULL && fp->next_address != NULL){
        fp = fp->next_address->next_address;
        sp = sp->next_address;
    }
    h1 = head;
    h2 = sp->next_address;
    sp->next_address = NULL;
...
Node* mergeSort_LL(Node* head){
    ...
    Node* t1 =  mergeSort_LL(a);
    Node* t2 = mergeSort_LL(b);

    Node* final = merge_LL(t1,t2);
